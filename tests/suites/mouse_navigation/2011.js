...
test('Selecting date from previous month while in January changes month and year displayed', function(){
    ...
    this.dp.update();
    datesEqual(this.dp.viewDate, moment([2011, 0, 1]));
    datesEqual(this.dp.dates.get(-1), moment([2011, 0, 1]));
    ...
    datesEqual(this.dp.viewDate, moment([2010, 11, 26]));
    datesEqual(this.dp.dates.get(-1), moment([2010, 11, 26]));
...
test('Selecting date from next month while in December changes month and year displayed', function(){
    ...
    this.dp.update();
    datesEqual(this.dp.viewDate, moment([2010, 11, 1]));
    datesEqual(this.dp.dates.get(-1), moment([2010, 11, 1]));
    ...
    target.click();
    equal(this.picker.find('.datepicker-days thead th.datepicker-switch').text(), 'January 2011');
    datesEqual(this.dp.viewDate, moment([2011, 0, 8]));
    datesEqual(this.dp.dates.get(-1), moment([2011, 0, 8]));
