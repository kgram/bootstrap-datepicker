...
test('Selecting date resets viewDate and date', function(){
    ...
    target.click();
    datesEqual(this.dp.viewDate, moment([2012, 2, 4]));
    datesEqual(this.dp.dates.get(-1), moment([2012, 2, 4]));

    // Re-rendered on click
    target = this.picker.find('.datepicker-days tbody td:first');
    equal(target.text(), '26'); // Should be Mar 26
});
...
test('Navigating to/from year view', function(){
    ...
    datesEqual(this.dp.viewDate, moment([2012, 3, 1])); // Apr 1
    datesEqual(this.dp.dates.get(-1), moment([2012, 2, 31]));
});
...
test('Navigating prev/next in decade view', function(){
    ...
    // Only viewDate modified
    datesEqual(this.dp.viewDate, moment([2022, 2, 31]));
    datesEqual(this.dp.dates.get(-1), moment([2012, 2, 31]));
    ...
    // Only viewDate modified
    datesEqual(this.dp.viewDate, moment([2002, 2, 31]));
    datesEqual(this.dp.dates.get(-1), moment([2012, 2, 31]));
});
...
test('Selecting date from previous month resets viewDate and date, changing month displayed', function(){
    ...
    datesEqual(this.dp.dates.get(-1), moment([2012, 1, 26]));
...
test('Selecting date from next month resets viewDate and date, changing month displayed', function(){
    ...
    datesEqual(this.dp.viewDate, moment([2012, 4, 5]));
    datesEqual(this.dp.dates.get(-1), moment([2012, 4, 5]));
